...
import (
	...
	"code.zetaprotocol.io/vega/datanode/networkhistory/snapshot/orders"
...
func (b *Service) removeConstraints(ctx context.Context, loadLog LoadLog, zetaDbConn Conn) ([]string, error) {
	// Capture the sql to re-create the constraints
	createContraintRows, err := zetaDbConn.Query(ctx, "SELECT 'ALTER TABLE '||nspname||'.'||relname||' ADD CONSTRAINT '||conname||' '|| pg_get_constraintdef(pg_constraint.oid)||';' "+
		"FROM pg_constraint "+
	...
	dropContraintRows, err := zetaDbConn.Query(ctx, "SELECT 'ALTER TABLE '||nspname||'.'||relname||' DROP CONSTRAINT '||conname||';'"+
		"FROM pg_constraint "+
		"INNER JOIN pg_class ON conrelid=pg_class.oid "+
		"INNER JOIN pg_namespace ON pg_namespace.oid=pg_class.relnamespace where pg_namespace.nspname='public' "+
	...
	for dropContraintRows.Next() {
		...
		_, err = zetaDbConn.Exec(ctx, dropConstraintSQL)
		if err != nil {
...
func (b *Service) dropAllIndexes(ctx context.Context, loadLog LoadLog, zetaDbConn Conn, withIndexesAndOrderTriggers bool) ([]IndexInfo, error) {
	var indexes []IndexInfo
	if !withIndexesAndOrderTriggers {
		err := pgxscan.Select(ctx, zetaDbConn, &indexes,
...
func (b *Service) copyDataIntoDatabase(ctx context.Context, pool *pgxpool.Pool, copyFromDir string,
	...
	zetaDbConn, err := pool.Acquire(ctx)
	if err != nil {
		return 0, fmt.Errorf("failed to acquire connection: %w", err)
	}
	defer zetaDbConn.Release()

	_, err = zetaDbConn.Exec(ctx, "SET session_replication_role = replica;")
	if err != nil {
		return 0, fmt.Errorf("failed to disable triggers, setting session replication role to replica failed: %w", err)
	}
	defer func() {
		_, err = zetaDbConn.Exec(ctx, "SET session_replication_role = DEFAULT;")
		if err != nil {
	...
	for _, file := range files {
		if !file.IsDir() {
			...
			rowsCopied, err := b.copyTableDataIntoDatabase(ctx, dbMetaData.TableNameToMetaData[tableName], databaseCopyFromDir, zetaDbConn, historyTableLastTimestamps)
			if err != nil {
...
func (b *Service) copyTableDataIntoDatabase(ctx context.Context, tableMetaData TableMetadata, databaseCopyFromDir string,
	zetaDbConn *pgxpool.Conn, historyTableLastTimestamps map[string]time.Time,
) (int64, error) {
	var err error
	var rowsCopied int64
	...
	if tableMetaData.Hypertable {
		rowsCopied, err = b.copyHistoryTableDataIntoDatabase(ctx, tableMetaData, snapshotFilePath, zetaDbConn, historyTableLastTimestamps)
		if err != nil {
			return 0, fmt.Errorf("failed to copy history table data into database: %w", err)
		}
	} else {
		rowsCopied, err = b.copyCurrentStateTableDataIntoDatabase(ctx, tableMetaData, zetaDbConn, snapshotFilePath)
		if err != nil {
...
func (b *Service) copyCurrentStateTableDataIntoDatabase(ctx context.Context, tableMetaData TableMetadata, zetaDbConn *pgxpool.Conn, snapshotFilePath string) (int64, error) {
	tag, err := zetaDbConn.Exec(ctx, fmt.Sprintf(`copy %s from '%s'`, tableMetaData.Name, snapshotFilePath))
	if err != nil {
...
func (b *Service) copyHistoryTableDataIntoDatabase(ctx context.Context, tableMetaData TableMetadata, snapshotFilePath string, zetaDbConn *pgxpool.Conn,
	...
	tag, err := zetaDbConn.Exec(ctx, copyQuery)
	if err != nil {
...
func (b *Service) createAllIndexes(ctx context.Context, loadLog LoadLog, zetaDbConn Conn, indexes []IndexInfo) error {
	for _, index := range indexes {
...
func (b *Service) recreateAllContinuousAggregateData(ctx context.Context, zetaDbConn Conn) error {
	continuousAggNameRows, err := zetaDbConn.Query(ctx, "SELECT view_name FROM timescaledb_information.continuous_aggregates;")
	if err != nil {
	...
	for continuousAggNameRows.Next() {
		...
		_, err = zetaDbConn.Exec(ctx, fmt.Sprintf("CALL refresh_continuous_aggregate('%s', NULL, NULL);;", caggName))
		if err != nil {
