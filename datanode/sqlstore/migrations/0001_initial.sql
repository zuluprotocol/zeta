...
CREATE INDEX ON orders (market_id, zeta_time DESC) where current=true;
CREATE INDEX ON orders (party_id, zeta_time DESC) where current=true;
CREATE INDEX ON orders (reference, zeta_time DESC) where current=true;
...
$$
    ...
    THEN
       ...
       VALUES(new.id, new.market_id, new.party_id, new.side, new.price,
              ...
              new.tx_hash, new.zeta_time, new.seq_num, true);
    END IF;
...
with cte_market_data_latest(market, zeta_time) as (
    select market, max(zeta_time)
    from market_data
    group by market
)
select md.market, md.tx_hash, md.zeta_time, seq_num, mark_price, best_bid_price, best_bid_volume, best_offer_price, best_offer_volume,
...
$$
BEGIN
    INSERT INTO markets_current(id,tx_hash,zeta_time,instrument_id,tradable_instrument,decimal_places,fees,opening_auction,price_monitoring_settings,liquidity_monitoring_parameters,trading_mode,state,market_timestamps,position_decimal_places,lp_price_range)
    VALUES(NEW.id,NEW.tx_hash,NEW.zeta_time,NEW.instrument_id,NEW.tradable_instrument,NEW.decimal_places,NEW.fees,NEW.opening_auction,NEW.price_monitoring_settings,NEW.liquidity_monitoring_parameters,NEW.trading_mode,NEW.state,NEW.market_timestamps,NEW.position_decimal_places,NEW.lp_price_range)
...
CREATE TABLE positions(
  ...
  tx_hash             BYTEA                    NOT NULL,
  zeta_time           TIMESTAMP WITH TIME ZONE NOT NULL,
  primary key (party_id, market_id, zeta_time)
);
