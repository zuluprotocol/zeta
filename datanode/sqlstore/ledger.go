...
func (ls *Ledger) Query(
	...
	if dateRange.Start != nil {
		whereDate = fmt.Sprintf("WHERE zeta_time >= %s", nextBindVar(&args, *dateRange.Start))
	}
	...
	if dateRange.End != nil {
		...
		whereDate = fmt.Sprintf("%s zeta_time < %s", whereDate, nextBindVar(&args, *dateRange.End))
	}
...
func (ls *Ledger) Export(
	...
	columns := []string{
		"ZetaTime", "AssetId", "SenderPartyId", "SenderMarketId", "SenderAccountType",
