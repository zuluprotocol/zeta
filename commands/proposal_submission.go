...
func checkDataSourceSpec(spec *zetapb.DataSourceDefinition, name string, parentProperty string) Errors {
	errs := NewErrors()
	if spec == nil {
...
func isBuiltInSpec(filters []*datapb.Filter) bool {
	...
	if strings.HasPrefix(filters[0].Key.Name, "zetaprotocol.builtin") && filters[0].Key.Type == datapb.PropertyKey_TYPE_TIMESTAMP {
		return true
	}
