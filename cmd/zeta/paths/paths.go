...
type Cmd struct {
	List    ListCmd    `command:"list" description:"List the location where files used by the Zeta applications are stored"`
...
func Paths(ctx context.Context, parser *flags.Parser) error {
	...
	var (
		short = "Manages the Zeta paths"
		long  = `
			Zeta applications store their configuration and their data at specific locations.
			...
			Zeta applications also support setting a custom location for these files using
			the --home flag. Contrary to the XDG Base Directory specification, this flag
			will group the cache, config, data and state folders under a "zeta" folder, at
